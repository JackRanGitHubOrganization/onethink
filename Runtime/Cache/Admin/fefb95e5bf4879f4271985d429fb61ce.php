...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('UserRole/add');?>">新 增</a>
            <button class="btn ajax-post" url="<?php echo U('UserRole/changeStatus',array('method'=>'resumeUser'));?>" target-form="ids">启 用</button>
            <button class="btn ajax-post" url="<?php echo U('UserRole/changeStatus',array('method'=>'forbidUser'));?>" target-form="ids">禁 用</button>
            <button class="btn ajax-post confirm" url="<?php echo U('UserRole/changeStatus',array('method'=>'deleteUser'));?>" target-form="ids">删 除</button>
        </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入角色昵称或者ID">
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["role_id"]); ?>" /></td>
			<td><?php echo ($vo["id"]); ?> </td>
			<td><?php echo ($vo["name"]); ?></td>
			<td><?php echo (time_format($vo["update_time"])); ?></td>
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('UserRole/changeStatus?method=forbidUser&id='.$vo['role_id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('UserRole/changeStatus?method=resumeUser&id='.$vo['role_id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<a href="<?php echo U('UserRole/changeStatus?method=deleteUser&id='.$vo['role_id']);?>" class="confirm ajax-get">删除</a>
                </td>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('UserRole/index');?>');
	</script>
